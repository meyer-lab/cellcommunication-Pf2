...
def makeFigure():
    ...
    X_epithelial = X[X.obs["broad_cell_type"] == "Dendritic Cells"]
    X_epithelial_send = X_epithelial.obsm["sc_B"][:, ccc_rise_cmp - 1]

    sns.violinplot(data=X_epithelial_send, ax=ax[0])
    ...
    sns.violinplot(data=X_epithelial_rec, ax=ax[1])
    ...
    X_epithelial = X[X.obs["broad_cell_type"] == "Macrophages"]
    X_epithelial_send = X_epithelial.obsm["sc_B"][:, ccc_rise_cmp - 1]

    sns.violinplot(data=X_epithelial_send, ax=ax[2])
    ax[2].set_ylim(-0.6, 0.2)
    ax[2].set_xlabel("Macrophages Weight Distribution")
    ax[2].set_ylabel(f"Sender Cell Component {ccc_rise_cmp} Association")

    X_epithelial_rec = X_epithelial.obsm["rc_C"][:, ccc_rise_cmp - 1]

    sns.violinplot(data=X_epithelial_rec, ax=ax[3])
    ax[3].set_ylim(-0.6, 0.2)
    ax[3].set_xlabel("Macrophages Weight Distribution")
