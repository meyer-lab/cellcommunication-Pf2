...
def makeFigure():
    ...
    celltype_counts = X.obs["broad_cell_type"].value_counts()
    print("Cell type counts:\n", celltype_counts)

    pal = sns.color_palette(palette="Set3")
    pal = pal.as_hex()
    plot_labels_pacmap(X, labelType="broad_cell_type", ax=ax[0], color_key=pal)
    pal = sns.color_palette("Set2")
    pal = [pal[0], pal[1], pal[2]]
    pal = [
        f"#{int(r * 255):02x}{int(g * 255):02x}{int(b * 255):02x}" for r, g, b in pal
    ]
    plot_labels_pacmap(X, labelType="ALADstatus", ax=ax[1], color_key=pal)
