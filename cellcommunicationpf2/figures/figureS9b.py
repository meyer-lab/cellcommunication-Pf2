...
def makeFigure():
    ...
    print(df[celltype_id].unique())
    df_dc_cd4 = df[df[celltype_id].isin(["Dendritic Cells", "NK cells"])]
    print(df_dc_cd4)
    ...
    print(df_dc_cd4_pivot)
    df_dc_cd4_pivot["DC_to_NK_Ratio"] = df_dc_cd4_pivot["Dendritic Cells"] / df_dc_cd4_pivot["NK cells"]
    ...
    sns.boxplot(
        data=df_dc_cd4_pivot, x=condition_id, y="DC_to_NK_Ratio", ax=ax[1], hue=condition_id,
    )
    
    # Statistical test between Control and ALAD
    from scipy.stats import mannwhitneyu
    control_data = df_dc_cd4_pivot[df_dc_cd4_pivot[condition_id] == "control"]["DC_to_NK_Ratio"]
    alad_data = df_dc_cd4_pivot[df_dc_cd4_pivot[condition_id] == "ALAD"]["DC_to_NK_Ratio"]
    ...
    # Plot scatter plot of DC proportion vs NK cell proportion where each point is a sample
    df_scatter = df[df[celltype_id].isin(["Dendritic Cells", "NK cells"])]
    ...
    ax[2].set_xlabel("NK cell proportion")
    ...
    correlation, p_value = spearmanr(df_scatter_pivot["NK cells"], df_scatter_pivot["Dendritic Cells"])
    ax[2].set_title(f"DC vs NK cell proportions: ρ={correlation:.2f}, p={p_value:.2g}")
