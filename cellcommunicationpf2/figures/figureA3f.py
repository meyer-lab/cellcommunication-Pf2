...
def makeFigure():
    ...
    X_epi_sender = add_obs_cmp_label(
        X_epi_sender, cmp=ccc_rise_cmp, pos=True, top_perc=10, type="sender"
    )
    ...
    X_epi_receiver = add_obs_cmp_label(
        X_epi_receiver, cmp=ccc_rise_cmp, pos=True, top_perc=10, type="receiver"
    )
    X_epi_receiver = add_obs_cmp_unique_one(X_epi_receiver, cmp=ccc_rise_cmp)
    ...
    for ligand, receptor in pairs:
        ...
        expr_product_df = expression_product_matrix(
            X_epi_sender, X_epi_receiver, ligand, receptor
        )
        ...
        for sample in X.obs["sample"].unique():
            ...
            sender_sample_mask = X_epi_sender.obs["sample"] == sample
            sender_cells_sample = X_epi_sender.obs_names[sender_sample_mask]

            # Get receiver cells from this sample
            receiver_sample_mask = X_epi_receiver.obs["sample"] == sample
            receiver_cells_sample = X_epi_receiver.obs_names[receiver_sample_mask]
            ...
            if len(sender_cells_sample) > 0 and len(receiver_cells_sample) > 0:
                ...
                avg_comm_score_no_label = sample_submatrix[
                    sample_submatrix.index.isin(
                        X_epi_sender.obs_names[X_epi_sender.obs["Label"] == "NoLabel"]
                    )
                ].values.mean()
